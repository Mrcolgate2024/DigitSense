...
def preprocess_image(image):
    ...
    # # Step 2: Centralization (find bounding box and resize)
    # rows = np.any(thresholded_image < 255, axis=1)
    # cols = np.any(thresholded_image < 255, axis=0)
    # rmin, rmax = np.where(rows)[0][[0, -1]]
    # cmin, cmax = np.where(cols)[0][[0, -1]]
    
    # cropped = thresholded_image[rmin:rmax+1, cmin:cmax+1]
    # cropped_resized = cv2.resize(cropped, (20, 20), interpolation=cv2.INTER_AREA)

    # # Step 3: Resize with border to 28x28
    # canvas_size = (28, 28)
    # canvas = np.full(canvas_size, 255, dtype=np.uint8)  # 255 for white background
    
    # y_offset = (canvas_size[0] - cropped_resized.shape[0]) // 2
    # x_offset = (canvas_size[1] - cropped_resized.shape[1]) // 2
